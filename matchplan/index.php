...
$args = explode('/', str_replace('matchplan/', '', strstr($_SERVER['REQUEST_URI'], 'matchplan/')));

// *** Vorgaben u. Variablen-Deklaration ***
$abfragen = array();
$matches = array();
$homepage = file_get_contents('http://www.fussball.de/ajax.club.matchplan.loadmore/-/id/00ES8GN8LS00005DVV0AG08LVUPGND5I/max/1000/show-venues/true/datum-von/2020-09-01/datum-bis/2020-10-15/wettkampftyp/1');
		...
		foreach ($abfragen as $abfragekey => $abfrage) {
			...
			switch($abfragekey) {
				...
				case "score-left":
					...
					$matches[$counter]["score_home"] = (strtoupper($matches[$counter]["teamname_home"]) == "SPIELFREI" || strtoupper($matches[$counter]["teamname_guest"]) == "SPIELFREI") ? "" : $matches[$counter]["score_home"];
					break;
				...
				case "score-right":
					...
					$matches[$counter]["score_guest"] = (strtoupper($matches[$counter]["teamname_home"]) == "SPIELFREI" || strtoupper($matches[$counter]["teamname_guest"]) == "SPIELFREI") ? "" : $matches[$counter]["score_guest"];
					break;
